...
/// let r = t.sin();
/// ```
pub fn sin<'a, S: Shape, E: Dtype, D: UnaryKernel<SinKernelOp, E> + 'a, T: Tape<'a, E, D>>(
    t: Tensor<S, E, D, T>,
) -> Tensor<S, E, D, T> {
    t.sin()
}

impl<'a, S: Shape, E: Dtype, D: UnaryKernel<SinKernelOp, E> + 'a, T: Tape<'a, E, D>> Tensor<S, E, D, T> {
    /// See [sin]
