...
/// let r = t.cos();
/// ```
pub fn cos<'a, S: Shape, E: Dtype, D: UnaryKernel<CosKernelOp, E> + 'a, T: Tape<'a, E, D>>(
    t: Tensor<S, E, D, T>,
) -> Tensor<S, E, D, T> {
    t.cos()
}

impl<'a, S: Shape, E: Dtype, D: UnaryKernel<CosKernelOp, E> + 'a, T: Tape<'a, E, D>> Tensor<S, E, D, T> {
    /// See [cos]
