...
/// let r = t.exp();
/// ```
pub fn exp<'a, S: Shape, E: Dtype, D: UnaryKernel<ExpKernelOp, E> + 'a, T: Tape<'a, E, D>>(
    t: Tensor<S, E, D, T>,
) -> Tensor<S, E, D, T> {
    t.exp()
}

impl<'a, S: Shape, E: Dtype, D: UnaryKernel<ExpKernelOp, E> + 'a, T: Tape<'a, E, D>> Tensor<S, E, D, T> {
    /// See [exp]
